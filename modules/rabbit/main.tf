...
################################################################
resource "hcloud_server" "rabbit_cluster_vm" {
  name        = var.rabbit_cluster_vm
  server_type = var.rabbit_server_type
  ...
  network {
    ...
    ip         = var.rabbit_static_ip # Static IP for opensearch server
  }
  ...
  user_data = templatefile("${path.module}/templates/userdata_rabbit.tpl", {
